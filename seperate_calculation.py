...
for id in [0, 1, 2, 3, 4, 5, 6, 8, 10 ]:
        print ( ' READ ' + str( id ))
        path_2x2x1_c= fix_path( database[str(id+16)+'/16_ML_Pb_on_4x4Ge111_constrained' ][ '2x2x1_path' ] )
        system1 = read( path_2x2x1_c + '/CONTCAR' )
        nPb.append( count_nPb( system1 ) )
        en_c = get_energy(path_2x2x1_c + '/OUTCAR' )
...
for id in [0, 1, 2, 3, 4, 5, 6, 8, 10 ]:
        print ( ' READ ' + str( id ))
        path_2x2x1_nc = fix_path( database[str(id + 16)+'/16_ML_Pb_on_4x4Ge111_not_constrained' ][ '2x2x1_path' ] )
        system2 = read( path_2x2x1_nc + '/CONTCAR' )
        en_nc = get_energy(path_2x2x1_nc + '/OUTCAR' )
...
for id in range( 1, len( nPb ) ):
	...
	path_1_c= fix_path( database[str(id+16)+'/16_ML_Pb_on_4x4Ge111_constrained' ][ '2x2x1_path' ] )
        system1_c = read( path_1_c +'/OUTCAR' )
	gamma1_c.append( calculate_gamma( system1_c, muy_c[ -1 ]  ) )
	path_1_nc = fix_path( database[str(id + 16)+'/16_ML_Pb_on_4x4Ge111_not_constrained' ][ '2x2x1_path' ] )
...
plt.plot(cov, np.array( dmuy2 ) - muy_Pb, "-o", label = "Pb no constrained")
...
plt.plot(cov, gamma1_nc, "-o", label = "Pb no constrained")
...
plt.plot(cov, gamma2_nc, "-o", label = "Pb no constrained")
plt.title("Pb Surface energy vs Pb coverage")
plt.xlabel("Pb coverage (ML)")
plt.ylabel("Surface Energy[J/$m^{2}$]")
plt.legend(loc = "best", ncol =1 )
